...
function findMeta(Entity) {
    ...
    entityMetaData[Entity.name] = __assign(__assign({}, tableInfo), { columns: setting.map(function (x) { return x.column; }), hooks: hooks });
    return entityMetaData[Entity.name];
}
exports.findMeta = findMeta;
function findMetaFromTableName(tableName) {
    for (var _i = 0, _a = Object.values(entityMetaData); _i < _a.length; _i++) {
        var meta = _a[_i];
        if (!meta) {
            continue;
        }
