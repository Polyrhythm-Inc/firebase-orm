...
function findMetaFromTableName(tableName) {
    ...
    }).filter(function (x) { return x.Entity == info.Entity; });
    var hooks = hookSettings.filter(function (x) { return x.getEntity() == info.Entity; }).map(function (x) { return x.hook; });
    var metaData = __assign(__assign({}, info), { columns: setting.map(function (x) { return x.column; }), hooks: hooks });
...
//     }
// }
function NestedFirebaseEntity(tableName) {
    var parentEntityGetters = [];
    for (var _i = 1; _i < arguments.length; _i++) {
        parentEntityGetters[_i - 1] = arguments[_i];
    }
    return function (constructor) {
        entityMetaInfo.push({
            tableName: tableName,
